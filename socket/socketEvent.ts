...
export const newMessage = (paylaod: any, off: boolean = false) => {
    const socket = getSocket();
    ...
    if (off) {
        // turn off listening to this event
        socket.off("newMessage", paylaod) // payload is the callback
    } else if (typeof paylaod == 'function') {
        socket.on("newMessage", paylaod) // payload as callback for this event
    } else {
        socket.emit("newMessage", paylaod) // sending payload as data
    }
}

export const getMessages = (paylaod: any, off: boolean = false) => {
    const socket = getSocket();
    ...
    if (off) {
        // turn off listening to this event
        socket.off("getMessages", paylaod) // payload is the callback
    } else if (typeof paylaod == 'function') {
        socket.on("getMessages", paylaod) // payload as callback for this event
    } else {
        socket.emit("getMessages", paylaod) // sending payload as data
    }
}